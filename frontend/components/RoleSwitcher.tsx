...
export default function RoleSwitcher() {
  ...
  const handleRoleSwitch = async () => {
    if (!userProfile || isToggling) return;
    
    setIsToggling(true);
    
    // Toggle role
    const newRole = userProfile.role === 'student' ? 'teacher' : 'student';
    ...
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 border border-valuto-green-100 shadow-lg max-w-3xl mx-auto">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-start gap-4 min-w-0 flex-1">
          <div className={`w-4 h-4 rounded-full mt-1 flex-shrink-0 ${userProfile.role === 'student' ? 'bg-blue-500' : 'bg-green-500'}`}></div>
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium text-gray-700 leading-tight">
              Currently viewing as: <span className="font-semibold capitalize text-valuto-green-600">{userProfile.role}</span>
            </p>
