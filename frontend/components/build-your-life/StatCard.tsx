...
export default function StatCard({ icon, label, value, color = "default", delay = 0 }: Props) {
  return (
    <motion.div
      ...
      className="rounded-lg border border-white/10 bg-[rgba(10,16,14,0.82)] p-3 backdrop-blur-xl md:p-4"
    >
      <div className="mb-1 flex items-center gap-2">
        <span className="text-lg">{icon}</span>
        <span className="font-display text-xs uppercase tracking-wider text-[#8ea097]">{label}</span>
      </div>
