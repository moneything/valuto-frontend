...
export default function Timeline({ history, currentAge }: Props) {
  return (
    <div className="mx-auto w-full max-w-3xl rounded-xl border border-white/8 bg-[rgba(27,31,39,0.94)] p-4 backdrop-blur-xl">
      <h3 className="mb-3 font-display text-xs uppercase tracking-widest text-[#94a8a0]">Life Timeline</h3>
            ...
            return (
              <div key={age} className="flex flex-col items-center">
                <div
                  ...
                  } ${hasEvent ? "ring-2 ring-amber-300/70 ring-offset-1 ring-offset-[#1B1F27]" : ""}`}
                />
                <span className={`mt-1 font-display text-xs ${reached ? "text-white" : "text-[#94a8a0]"}`}>{age}</span>
              </div>
            );
          })}
        </div>
